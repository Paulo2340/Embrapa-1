...
grupo_1 <- wdata %>% filter(predito == "Neutro")
...
### Mudança FO
ggplot(tabela_sumarizada1, aes(reorder(genotipo, soma_mudanca), soma_mudanca, ymin = 0, ymax = soma_mudanca)) + geom_pointrange() + 
geom_point() + theme_bw() + theme(legend.position = "none") + coord_flip() + 
labs(title = "Porcentagem de Mudança para Doença FO no efeito climático Neutro", x = "Genotipo", y = "Porcentagem de mudança") + geom_text(aes(label = round(soma_mudanca, 2)), hjust = -0.5)

x11()
ggplot(tabela_sumarizada1 , aes(reorder(genotipo, soma_nao_mudanca), soma_nao_mudanca, ymin = 0, ymax = soma_nao_mudanca)) + geom_pointrange() + 
  geom_point() + theme_bw() + theme(legend.position = "none") + coord_flip() + 
  labs(title = "Porcentagem de Não Mudança para Doença FO no efeito climático Neutro", x = "Genotipo", y = "Porcentagem de não mudança") + geom_text(aes(label = round(soma_nao_mudanca, 2)), hjust = -0.5)
...
ggplot(tabela_sumarizada1 %>% filter(soma_mudanca != 0), aes(reorder(genotipo, soma_mudanca), soma_mudanca, ymin = 0, ymax = soma_mudanca)) + geom_pointrange() + 
  geom_point() + theme_bw() + theme(legend.position = "none") + coord_flip() + 
  labs(title = "Porcentagem de Mudança para Doença FS no efeito Neutro", x = "Genotipo", y = "Porcentagem de mudança") + geom_text(aes(label = round(soma_mudanca, 2)), hjust = -0.5)
...
ggplot(tabela_sumarizada1 %>% filter(soma_nao_mudanca != 0), aes(reorder(genotipo, soma_nao_mudanca), soma_nao_mudanca, ymin = 0, ymax = soma_nao_mudanca)) + geom_pointrange() + 
  geom_point() + theme_bw() + theme(legend.position = "none") + coord_flip() + 
  labs(title = "Porcentagem de Não Mudança para Doença FS no efeito neutro", x = "Genotipo", y = "Porcentagem de não mudança") + geom_text(aes(label = round(soma_nao_mudanca, 2)), hjust = -0.5)
