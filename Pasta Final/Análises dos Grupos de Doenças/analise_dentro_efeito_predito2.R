...
grupo_1 <- wdata %>% filter(predito == 2)
...
ggplot(tabela_sumarizada1, aes(reorder(genotipo, soma_mudanca), soma_mudanca, ymin = 0, ymax = soma_mudanca)) + geom_pointrange() + 
  ...
  labs(title = "Porcentagem de Mudança para Doença FO", x = "Genotipo", y = "Porcentagem de mudança") + geom_text(aes(label = round(soma_mudanca, 2)), hjust = -0.5)
...
ggplot(tabela_sumarizada1, aes(reorder(genotipo, soma_nao_mudanca), soma_nao_mudanca, ymin = 0, ymax = soma_nao_mudanca)) + geom_pointrange() + 
  geom_point() + theme_bw() + theme(legend.position = "none") + coord_flip() + 
  labs(title = "Porcentagem de Não Mudança para Doença FO", x = "Genotipo", y = "Porcentagem de não mudança") + geom_text(aes(label = round(soma_nao_mudanca, 2)), hjust = -0.5)
...
ggplot(tabela_sumarizada1, aes(reorder(genotipo, soma_mudanca), soma_mudanca, ymin = 0, ymax = soma_mudanca)) + geom_pointrange() + 
  geom_point() + theme_bw() + theme(legend.position = "none") + coord_flip() + 
  labs(title = "Porcentagem de Mudança para Doença FS", x = "Genotipo", y = "Porcentagem de mudança") + geom_text(aes(label = round(soma_mudanca, 2)), hjust = -0.5)
